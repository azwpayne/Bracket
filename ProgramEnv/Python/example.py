...
def train_classifier_simple_v2(
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Time / epoch without evaluation: {elapsed:.10f} min")
        with torch.no_grad():  # save memory during inference
            ...
            print(
                f"Epoch: {epoch + 1:03d}/{num_epochs:03d} "
                f"| Train: {train_acc :.10f}% "
                f"| Validation: {valid_acc :.10f}% "
                f"| Best Validation "
                f"(Ep. {best_epoch:03d}): {best_valid_acc :.10f}%"
            )

        elapsed = (time.time() - start_time) / 60
        print(f"Time elapsed: {elapsed:.10f} min")
    ...
    elapsed = (time.time() - start_time) / 60
    print(f"Total Training Time: {elapsed:.10f} min")

    test_acc = compute_accuracy(model, test_loader, device=device)
    print(f"Test accuracy {test_acc :.10f}%")

    elapsed = (time.time() - start_time) / 60
    print(f"Total Time: {elapsed:.10f} min")
